...
const PANEL =
  "rounded-3xl border border-white/10 bg-white/6 shadow-[0_20px_80px_rgba(0,0,0,0.5)] backdrop-blur-xl";
const chipBase =
...
function Label({ children, tone = "neutral" }) {
  const tones = {
    neutral: "border-white/15 bg-white/6 text-white/80",
...
function StarField() {
  ...
  useEffect(() => {
    ...
    let h = 0;
    const resize = () => {
      w = canvas.width = canvas.clientWidth * Math.min(window.devicePixelRatio || 1, 2);
      h = canvas.height = canvas.clientHeight * Math.min(window.devicePixelRatio || 1, 2);
  ...
  return (
    <div className={cn(PANEL, "p-6 sm:p-8")}> 
      <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
              ...
              {items.map((item) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="font-semibold">{item.d}D — {item.name}</div>
        ...
        <div className="relative flex min-h-[420px] items-center justify-center overflow-hidden rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_center,rgba(101,163,255,0.16),rgba(10,10,25,0.95)_58%)] p-6">
          ...
          <div className="absolute bottom-6 left-6 right-6 rounded-2xl border border-white/10 bg-black/30 p-4 backdrop-blur-md">
  ...
  return (
    <div className="grid h-64 w-64 place-items-center">
      <svg viewBox="0 0 200 200" className="h-56 w-56">
        <motion.line x1="25" y1="100" x2="175" y2="100" stroke="white" strokeWidth="6" strokeLinecap="round" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 1 }} />
      </svg>
    </div>
  );
}
function SquareIcon() {
  return (
    <div className="grid h-64 w-64 place-items-center">
      <svg viewBox="0 0 200 200" className="h-56 w-56">
        <motion.rect x="40" y="40" width="120" height="120" rx="10" fill="none" stroke="white" strokeWidth="4" initial={{ scale: 0.8, rotate: -8, opacity: 0 }} animate={{ scale: 1, rotate: 0, opacity: 1 }} />
      </svg>
    </div>
  );
}
    ...
    <div className="grid h-64 w-64 place-items-center">
      <svg viewBox="0 0 220 220" className="h-60 w-60">
        <g fill="none" stroke="white" strokeWidth="3">
          ...
          <path d="M85 45 L165 45 L165 125 L85 125 Z" opacity="0.78" />
    ...
    <div className="grid h-64 w-64 place-items-center">
      <svg viewBox="0 0 220 220" className="h-60 w-60">
        <g fill="none" stroke="white" strokeWidth="2.5">
          <motion.rect x="42" y="48" width="90" height="90" rx="6" initial={{ rotate: 0 }} animate={{ rotate: 0 }} />
          <motion.rect x="88" y="82" width="90" height="90" rx="6" initial={{ rotate: 0 }} animate={{ rotate: 0 }} opacity="0.82" />
      ...
      <div className="mt-2 text-center text-sm text-white/55">A cube projected into 3D is only a shadow of a 4D shape.</div>
    </div>
  );
}
function HypercubeIcon({ level }) {
  return (
    <div className="grid h-64 w-64 place-items-center">
      ...
      <div className="mt-2 text-center text-sm text-white/55">{level}D remains mathematically drawable even when it is not intuitively visualizable.</div>
    </div>
  );
}
      ...
      <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        <div>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <button className="rounded-full border border-white/10 bg-white/8 px-3 py-2 text-sm text-white/75" onClick={() => setPhase((s) => Math.max(0, s - 1))}>Previous slice</button>
            <button className="rounded-full border border-white/10 bg-white/8 px-3 py-2 text-sm text-white/75" onClick={() => setPhase((s) => Math.min(phases.length - 1, s + 1))}>Next slice</button>
          </div>
        </div>
          ...
          <div className="relative h-[360px] overflow-hidden rounded-3xl bg-[linear-gradient(180deg,rgba(16,18,32,0.9),rgba(4,7,16,0.96))]">
            <svg viewBox="0 0 300 240" className="absolute inset-0 h-full w-full">
              ...
              <g>
                ...
                <motion.circle cx={p.x} cy="120" r={p.r} fill="rgba(125,211,252,0.22)" stroke="rgba(125,211,252,0.9)" strokeWidth="3" animate={{ cx: p.x, r: p.r }} transition={{ duration: 0.75, ease: "easeInOut" }} />
                <path d="M20 120 H280" stroke="rgba(255,255,255,0.14)" strokeDasharray="8 8" />
                <text x="24" y="238" fill="rgba(255,255,255,0.55)" fontSize="11">Only the slice is visible to Flatland observers.</text>
              </g>
            </svg>
            <div className="absolute left-4 top-4 rounded-full bg-black/40 px-3 py-1 text-xs text-white/70">A 3D object becomes a 2D event.</div>
            <div className="absolute right-4 bottom-4 rounded-full bg-black/40 px-3 py-1 text-xs text-white/70">What would a 4D object become in our 3D world?</div>
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_0.95fr] lg:items-center">
        <div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <RangeCard title="Mass of the object" value={`${mass} M☉`} min={3} max={40} onChange={setMass} label="Solar masses" />
            <RangeCard title="Observer distance" value={`${distance} units`} min={4} max={100} onChange={setDistance} label="Closer means stronger curvature" />
          </div>
        ...
        <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_center,rgba(20,24,51,0.95),rgba(4,7,16,1)_68%)] p-5">
          <svg viewBox="0 0 640 560" className="h-full w-full">
            <defs>
              <radialGradient id="bhGlow" cx="50%" cy="50%" r="70%">
                <stop offset="0%" stopColor="rgba(145,94,255,0.28)" />
                <stop offset="30%" stopColor="rgba(74,144,226,0.18)" />
            ...
            <g opacity="0.55" stroke="url(#gridWaves)">
              {[...Array(10)].map((_, i) => (
                <path key={i} d={`M40 ${110 + i * 30} C 180 ${90 + i * 30}, 380 ${140 + i * 24}, 600 ${100 + i * 30}`} fill="none" opacity={0.16 + i * 0.03} />
              ))}
            </g>
            <g>
              ...
              <motion.circle cx="320" cy="280" r="104" fill="none" stroke="rgba(125,211,252,0.85)" strokeWidth="2" strokeDasharray="10 10" animate={{ rotate: 360 }} transition={{ duration: 18, repeat: Infinity, ease: "linear" }} style={{ originX: "320px", originY: "280px" }} />
              <g transform="translate(480 120)">
                <motion.path d="M0 0 C 40 18, 70 42, 106 74" fill="none" stroke="rgba(255,255,255,0.85)" strokeWidth="4" strokeLinecap="round" strokeDasharray="240" strokeDashoffset="0" animate={{ pathLength: [0, 1] }} transition={{ duration: 1.4, repeat: Infinity, repeatType: "reverse" }} />
                <circle cx="0" cy="0" r="7" fill="white" />
                <text x="0" y="108" fill="rgba(255,255,255,0.65)" fontSize="14">Spacetime bends</text>
              </g>
            </g>
          </svg>
          <div className="absolute left-5 top-5 rounded-2xl border border-white/10 bg-black/35 p-4 backdrop-blur-md">
            ...
            <div className="mt-1 text-xs text-white/50">Approximate for non-rotating idealized objects.</div>
          </div>
          <div className="absolute bottom-5 left-5 right-5 rounded-2xl border border-white/10 bg-black/35 p-4 backdrop-blur-md">
            ...
            <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/8">
              <div className="h-full rounded-full bg-gradient-to-r from-cyan-300 via-violet-400 to-fuchsia-500" style={{ width: `${curvature}%` }} />
            </div>
          </div>
        </div>
      </div>
...
function MathSection() {
  const cards = [
    ...
    { eq: "d = √((x₂−x₁)² + (y₂−y₁)²)", title: "Distance formula", tone: "supported", desc: "How far apart are two points? This is geometry turned into arithmetic." },
    ...
    { eq: "R_{μν} − ½g_{μν}R + Λg_{μν} = (8πG/c⁴)T_{μν}", title: "Einstein field equations", tone: "established", desc: "Matter-energy tells spacetime how to curve; curvature tells matter how to move." },
    { eq: "r_s = 2GM/c²", title: "Schwarzschild radius", tone: "supported", desc: "The horizon scale for a non-rotating black hole idealization." },
  ];
  return (
    <div className={cn(PANEL, "p-6 sm:p-8")}>
      <SectionHeading
        ...
        subtitle="Never assume prior knowledge. Start with points and distances. Then extend to vectors, curvature, tensors, and relativity. The formulas are not decoration. They are compressed ways of asking precise questions."
      />
      <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {cards.map((card) => (
          <div key={card.title} className="rounded-3xl border border-white/10 bg-black/25 p-5">
            <Label tone={card.tone}>{card.tone.replace(/^[a-z]/, (m) => m.toUpperCase())}</Label>
      ...
      <div className="mt-6 rounded-3xl border border-white/10 bg-gradient-to-r from-cyan-500/10 via-violet-500/10 to-fuchsia-500/10 p-5">
        <p className="text-sm leading-7 text-white/72">
          <Label tone="model">Mathematical Model</Label> A tensor is like a multi-directional data structure for geometry and physics. A curvature tensor tells us how space changes when you move in different directions. In general relativity, this curvature is not merely a background stage; it is the stage itself.
        </p>
      </div>
    </div>
  );
...
function TheoryMatrix() {
  const theories = [
    ...
    ["Black String", "Higher-dimensional mathematical solution", "Useful in theoretical gravity, not observed astrophysically."],
  ...
  return (
    <div className={cn(PANEL, "p-6 sm:p-8")}>
      ...
      <div className="mt-8 overflow-hidden rounded-3xl border border-white/10">
        <div className="grid grid-cols-[1.2fr_0.75fr_1.1fr] bg-white/5 px-5 py-3 text-xs font-semibold uppercase tracking-[0.2em] text-white/55">
          <div>Theory</div>
          <div>Status</div>
          <div>What it suggests</div>
        </div>
        <div className="divide-y divide-white/10 bg-black/25">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <div className={cn(PANEL, "p-6 sm:p-8")}>
        ...
        <div className="mt-6 space-y-3">
          {objections.map((text) => (
            <div key={text} className="rounded-2xl border border-white/10 bg-black/25 p-4 text-sm leading-7 text-white/72">
              {text}
            </div>
          ))}
        </div>
            ...
            {events.map(([name], idx) => (
              <button
                ...
                className={cn(
                  ...
                  idx === active ? "bg-white/10 text-white" : "text-white/60 hover:bg-white/5"
                )}
              >
                <span className={cn("grid h-7 w-7 place-items-center rounded-full border text-xs", idx === active ? "border-cyan-300/50 bg-cyan-300/12" : "border-white/10 bg-white/5")}>{idx + 1}</span>
                <span className="font-semibold">{name}</span>
              </button>
            ))}
          </div>
        </div>
        <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-cyan-400/10 via-violet-500/10 to-black/25 p-6">
          ...
          <div className="mt-8 h-60 rounded-3xl border border-white/10 bg-black/30 p-4">
            <svg viewBox="0 0 500 260" className="h-full w-full">
              ...
              <text x="22" y="28" fill="rgba(255,255,255,0.55)" fontSize="12">A timeline is a memory of questions, not just dates.</text>
            </svg>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
        <div className="rounded-3xl border border-white/10 bg-black/25 p-5">
          ...
          <input
            ...
            className="mt-4 w-full accent-cyan-300"
          />
          <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm leading-7 text-white/70">
        ...
        <div className="relative h-[420px] overflow-hidden rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_center,rgba(104,91,255,0.16),rgba(2,4,12,0.96)_68%)]">
          <svg viewBox="0 0 560 380" className="h-full w-full">
            ...
            <text x="24" y="30" fill="rgba(255,255,255,0.55)" fontSize="12">The visible circle is only a slice of something larger.</text>
            {[...Array(7)].map((_, i) => (
              <circle key={i} cx={110 + i * 58} cy="315" r={6 + (i % 3) * 2} fill="rgba(255,255,255,0.45)" opacity={0.4 + i * 0.08} />
            ))}
          </svg>
          <div className="absolute left-5 top-5 rounded-full bg-black/35 px-3 py-1 text-xs text-white/70">Cross-section simulator</div>
          <div className="absolute bottom-5 right-5 rounded-full bg-black/35 px-3 py-1 text-xs text-white/70">This is an analogy — not evidence.</div>
        </div>
      </div>
    </div>
  );
      ...
      <div className="grid gap-0 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="relative min-h-[420px] p-6 sm:p-8 lg:p-10">
          ...
          <div className="relative">
            ...
            <div className="mt-8 rounded-3xl border border-white/10 bg-black/25 p-5 text-sm leading-7 text-white/68">
        ...
        <div className="relative min-h-[420px] overflow-hidden border-t border-white/10 lg:border-l lg:border-t-0">
          <svg viewBox="0 0 640 420" className="absolute inset-0 h-full w-full">
            ...
            <text x="44" y="378" fill="rgba(255,255,255,0.55)" fontSize="13">A black hole may be an answer. Or it may be a doorway to a larger question.</text>
          </svg>
        </div>
      </div>
    </div>
          ...
          <div className={cn(SECTION, "flex min-h-screen items-center py-20")}> 
            <div className="max-w-4xl">
              ...
              <div className="mt-8 flex flex-wrap items-center gap-4">
                ...
                <div className="inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/6 px-5 py-3 text-sm text-white/70 backdrop-blur-xl">
        ...
        <section className={SECTION}>
          <div className={cn(PANEL, "p-6 sm:p-8")}>
            <div className="max-w-4xl">
              ...
              <h3 className="mt-4 text-2xl font-semibold text-white sm:text-3xl">Could a higher-dimensional object leave only a 3D footprint?</h3>
